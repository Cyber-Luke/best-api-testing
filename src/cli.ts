...
async function promptForProjectConfig(): Promise<ProjectConfig> {
  ...
  const auth: ProjectConfig["auth"] = { type: responses.authType };
  ...
  if (responses.customHeaders && responses.customHeaders.trim()) {
    try {
      ...
      console.log(
        chalk.yellow("⚠️  Invalid headers JSON, using empty headers")
      );
    }
  }
...
async function initProject(projectName?: string) {
  ...
  writeFileSync(
    path.join(projectDir, "integration-test.config.json"),
...
async function runTests(pattern?: string) {
  ...
  try {
    ...
    if (!existsSync(testsDir)) {
      ...
      console.log(
        chalk.yellow('💡 Run "npm run build" first to compile your tests.')
      );
      process.exit(1);
    }
  ...
  try {
    switch (command) {
      ...
      case "help":
        ...
        console.log(
          chalk.gray("  best init my-api-tests  ") +
            chalk.dim("# Create new project")
        );
        console.log(
          chalk.gray("  best init               ") +
        ...
        console.log(
          chalk.gray("  best run --pattern=user ") +
